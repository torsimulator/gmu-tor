...
send_control_event_string(uint16_t event, event_format_t which,
                          const char *msg)
{
    //printf("call send_control_event_string\n");
  smartlist_t *conns = get_connection_array();
  (void)which;
                            ...
                            const char *body)
{
  ...
  if (ENTRY_TO_CONN(ap_conn)->state != AP_CONN_STATE_CONTROLLER_WAIT &&
    ...
    connection_write_str_to_buf(
        ...
        //printf("ap_conn->base.state is %d", ap_conn->_base.state);
...
control_event_circuit_status(origin_circuit_t *circ, circuit_status_event_t tp,
                             int reason_code)
{
    //printf("control_event_circuit_status status is %d \n", tp);
  const char *status;
  char reasons[64] = "";
...

  {
    ...
    const char *sp = strlen(circdesc) ? " " : "";
        //printf("control_event_circuit_status calling send control event \n");
