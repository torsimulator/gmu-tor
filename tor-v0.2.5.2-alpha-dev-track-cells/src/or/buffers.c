...
fetch_var_cell_from_buf(buf_t *buf, var_cell_t **out, int linkproto)
{
  ...
  *out = NULL;
  ...
  command = get_uint8(hdr + circ_id_len + 8); /* JG: Different offset with unique IDs */
  if (!(cell_command_is_var_length(command, linkproto)))
    return 0;

  length = ntohs(get_uint16(hdr + circ_id_len + 9));
  ...
  result->unique_id = ntohl(get_uint32(hdr+circ_id_len+4));
