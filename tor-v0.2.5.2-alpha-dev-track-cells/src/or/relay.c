...
cell_queue_clear(cell_queue_t *queue)
{
  ...
  while ((cell = TOR_SIMPLEQ_FIRST(&queue->head))) {
    ...
    /* JG: QUEUE_CLEAR tracker message */
    cell_t unpacked_cell;
    cell_unpack(&unpacked_cell, cell->body, 1);
    if(unpacked_cell.unique_id) {
      log_notice(LD_GENERAL, "[CELL_TRACK] [CIRC_QUEUE_CLEAR] circ_id=%d unique_id=%8.8X command=%d",
						unpacked_cell.circ_id, unpacked_cell.unique_id, unpacked_cell.command);
	}
...
cell_queue_pop(cell_queue_t *queue)
{
  ...
  /* JG: QUEUE_POP tracker message */
  cell_t unpacked_cell;
  cell_unpack(&unpacked_cell, cell->body, 1);

  if(unpacked_cell.unique_id) {
	 log_notice(LD_GENERAL, "[CELL_TRACK] [CIRC_QUEUE_POP] circ_id=%d unique_id=%8.8X command=%d",
					 unpacked_cell.circ_id, unpacked_cell.unique_id, unpacked_cell.command);
  }
                             ...
                             streamid_t fromstream)
{
  ...
  /* JG: CIRC_QUEUE_APPEND tracker message */
  if(cell->unique_id) {
    log_notice(LD_GENERAL, "[CELL_TRACK] [CIRC_QUEUE_APPEND] circ_id=%d unique_id=%8.8X command=%d",
...
channel_consider_sending_flowcontrol_cell(int cell_direction, int nBuffer, circuit_t *circ, channel_t *chan)
{
    ...
    log_debug(LD_CHANNEL," to channel %p, with global ID "
    ...
    */
    ...
    if(cell_direction == CELL_DIRECTION_IN){    //Towards Entry
        ...
        log_debug(LD_CHANNEL,"CELL_DIRECTION_OUT");
        or_circ = TO_OR_CIRCUIT(circ);
