...
cell_pack(packed_cell_t *dst, const cell_t *src, int wide_circ_ids)
{
  ...
  set_uint32(dest, CELL_TRACKER_MAGIC); /* RJ: magic for cell tracker */
  set_uint32(dest+4, htonl(src->unique_id)); /* RJ: uid for cell tracker */
  dest += 8;
...
cell_unpack(cell_t *dest, const char *src, int wide_circ_ids)
{
  ...
  uint32_t magic = ntohl(get_uint32(src)); /* RJ: magic for cell tracker */
  dest->unique_id = ntohl(get_uint32(src+4)); /* RJ: uid for cell tracker */
  src += 8;
...
var_cell_pack_header(const var_cell_t *cell, char *hdr_out, int wide_circ_ids)
{
  ...
  set_uint32(hdr_out, CELL_TRACKER_MAGIC); /* RJ: magic for cell tracker */
  set_uint32(hdr_out+4, htonl(cell->unique_id)); /* RJ: uid for cell tracker */
  hdr_out += 8;
...
var_cell_new(uint16_t payload_len)
{
  ...
  /* JG: Attempt to set the cells unique ID */
...
connection_or_process_cells_from_inbuf(or_connection_t *conn)
{
  ...
  while (1) {
    ...
    if (connection_fetch_var_cell_from_buf(conn, &var_cell)) {
      ...
      /* JG: CONN_IN_BUF_READ_VAR tracker message */
      if(var_cell->unique_id) {
        log_notice(LD_GENERAL, "[CELL_TRACK] [CONN_BUF_READ_VAR] circ_id=%d unique_id=%8.8X command=%d",
        		var_cell->circ_id, var_cell->unique_id, var_cell->command);
      }
      ...
      /* JG: CONN_IN_BUF_READ tracker message */
      if(cell.unique_id) {
        log_notice(LD_GENERAL, "[CELL_TRACK] [CONN_BUF_READ] circ_id=%d unique_id=%8.8X command=%d",
        		cell.circ_id, cell.unique_id, cell.command);
      }
...
connection_or_send_netinfo(or_connection_t *conn)
{
  ...
  /* JG: Attempt to set the cells unique ID */
  cell_set_unique_id(&cell, NULL, "CREATE_CELL_NETINFO");
