...
channel_write_packed_cell(channel_t *chan, packed_cell_t *packed_cell)
{
  ...
  /* JG: CONN_BUF_WRITE tracker message */
  cell_t unpacked_cell;
  cell_unpack(&unpacked_cell, packed_cell->body, 1);
  if(unpacked_cell.unique_id) {
      log_notice(LD_GENERAL, "[CELL_TRACK] [CONN_BUF_WRITE] circ_id=%d unique_id=%8.8X command=%d",
...
/**
* Send N23 Flow Control Cell on a channel
*/
void
