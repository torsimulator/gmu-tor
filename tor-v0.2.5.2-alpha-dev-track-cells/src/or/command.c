...
static void command_process_flowcontrol_cell(cell_t *cell, channel_t *chan);
...
#define PROCESS_CELL(tp, cl, cn) command_process_ ## tp ## _cell(cl, cn)
  ...
  switch (cell->command) {
    ...
    case CELL_FLOWCONTROL:
      ...
      log_debug(LD_OR,"FLOWCONTROL CELL RECEIVED!!");
      break;
    default:
...
command_process_relay_cell(cell_t *cell, channel_t *chan)
{
  ...
  /* JG: If the cell has a unique ID, mark so in circuit */
  if(!circ->stamp_cells && cell->unique_id) {
	circ->stamp_cells = 1;
	log_notice(LD_GENERAL, "Enabling unique IDs on circ %d from cell %8.8X", cell->circ_id, cell->unique_id);
  }
