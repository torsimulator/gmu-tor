...
#define N2 (get_options()->N2Initial)
...
/* JG: Size of extra data in headers */
#define CELL_TRACKING_DATA_SIZE 8

/** Number of bytes in a cell, minus cell header. */
#define CELL_PAYLOAD_SIZE 509
/** Number of bytes in a cell transmitted over the network, in the longest
 * form */
/*JG: Add 8 for the new unique IDs */
...
typedef struct cell_t {
  ...
  uint8_t command; /**< Type of the cell: one of CELL_PADDING, CELL_CREATE,
                    * CELL_DESTROY, etc */
  uint32_t unique_id; /**< JG: Unique ID for tracking cells */
...
typedef struct packed_cell_t {
  ...
  uint32_t unique_id; /** JG: The unique ID of the packed cell */
...
/* JG: Magic value to stick in cells so they can be tracked in shadow */
#define CELL_TRACKER_MAGIC 0x8B5ED14Fu
...
typedef struct circuit_t {
  ...
  smartlist_t *testing_cell_stats;
    ...
    *   N23 Modification
    **/
    int credit_balance_n;
    uint32_t cells_fwded_n;
  /* JG: Whether or not to include unique IDs in cells */
  int stamp_cells;
} circuit_t;
...
#define REND_TOKEN_LEN DIGEST_LEN
  ...
  * N23 Modification
  */
  int credit_balance_p;
  uint32_t cells_fwded_p;
...
#define MAX_MAX_CLIENT_CIRCUITS_PENDING 1024
  ...
  /* JG: Cell track config option, needed so clients start stamping unique ids
   * exits will stamp return cells when it sees a unique id from the client */
  int InitiateCellTracking;
  /* RJ: this sets the sample rate for tagging new cells with unique ids.
   * clients and exits should set this so that new cells they create get tagged.
   * middle relays will always print messages for any cell that has been already
   * given a unique id from the client/exit.
   */
  double TrackCellSampleRate;

    /** If 1, use the N23 flow control algorithm. */
  int UseN23;
